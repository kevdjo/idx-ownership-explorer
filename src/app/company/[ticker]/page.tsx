...
export default function CompanyPage() {
  ...
  const { ticker: companyTicker, company: companyName, shareholders } = company;
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span className="inline-block px-4 py-2 text-xl font-bold text-blue-600 dark:text-blue-400 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">
              {formatNumber(shareholders.reduce((sum, s) => sum + s.total_shares, 0))}
            </p>
          </div>
          <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">Top Shareholder</p>
            <p className="text-lg font-bold text-purple-600 dark:text-purple-400 mt-1 truncate">
              {shareholders[0]?.name}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8">
          <NetworkGraph
            ...
            centerNode={{ type: "company", name: companyName, ticker: companyTicker }}
          />
        </div>
                ...
                {shareholders.map((shareholder, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link
                        href={`/investor/${encodeURIComponent(shareholder.name)}`}
                    ...
                    <td className="px-6 py-4 text-sm text-right text-gray-600 dark:text-gray-300 font-mono">
                      {formatNumber(shareholder.total_shares)}
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-right text-gray-600 dark:text-gray-300 font-mono">
                      {formatNumber(shareholder.holding_scrip)}
                      <span className="ml-1 text-xs text-gray-400">
                        ({shareholder.holding_scrip_percentage.toFixed(1)}%)
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-gray-600 dark:text-gray-300 font-mono">
                      {formatNumber(shareholder.holding_scripless)}
                      <span className="ml-1 text-xs text-gray-400">
                        ({shareholder.holding_scripless_percentage.toFixed(1)}%)
                      </span>
                    </td>
                  </tr>
                ))}
